...
local AddValues = function(Section, Object, Exceptions, Prefix)
    ...
    -- Filtra apenas índices ordenáveis (strings ou números)
    ...
    -- Ordena garantindo que sempre seja comparável
    ...
    for Index, Value in next, Copy do
        if typeof(Value) ~= "boolean" or (Exceptions and table.find(Exceptions, Index)) then
            continue
        end

        Section:Toggle({
            Name = string.gsub(Index, "(%l)(%u)", function(...)
    ...
    for Index, Value in next, Copy do
        if typeof(Value) ~= "Color3" or (Exceptions and table.find(Exceptions, Index)) then
            continue
        end
